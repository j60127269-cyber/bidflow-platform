-- Enhanced Notification Queue System (Fixed Version)
...
DROP INDEX IF EXISTS idx_notification_queue_unique;
...
-- 8. Drop existing trigger if it exists, then create new one
...
DO $$
BEGIN
    RAISE NOTICE 'Enhanced notification queue system migration completed successfully!';
    RAISE NOTICE 'Created notification_queue table with status tracking';
    RAISE NOTICE 'Created contract_versions table for version management';
    RAISE NOTICE 'Created admin dashboard view and management functions';
    RAISE NOTICE 'All existing objects handled gracefully';
END $$;
