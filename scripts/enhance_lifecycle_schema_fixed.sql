-- Enhanced Contract Lifecycle and Bidding Intelligence Schema (FIXED)
...
DROP POLICY IF EXISTS "Allow authenticated users to read contract_lifecycle_events" ON contract_lifecycle_events;
DROP POLICY IF EXISTS "Allow admins to manage contract_lifecycle_events" ON contract_lifecycle_events;
...
DROP TRIGGER IF EXISTS update_contract_lifecycle_events_updated_at ON contract_lifecycle_events;
...
-- 9. CREATE TRIGGER TO AUTO-UPDATE BIDDING STATS (IF NOT EXISTS)
DROP TRIGGER IF EXISTS trigger_update_bidding_stats ON competitor_bids;
